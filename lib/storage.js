...
export async function saveMediaFile(tempPath, storedFilename, mimeType, kind) {
  ...
  try {
    if (kind === "video") {
      ...
      thumbBuffer = await sharp(frame).rotate().resize({ width: THUMB_WIDTH, withoutEnlargement: true }).webp({ quality: 70 }).toBuffer();
      durationSeconds = duration;
    } else {
      thumbBuffer = await sharp(tempPath).rotate().resize({ width: THUMB_WIDTH, withoutEnlargement: true }).webp({ quality: 70 }).toBuffer();
    }
  } catch (e) {
...
export async function readFileBuffer(storedFilename) {
  return DRIVER === "s3" ? s3ReadBuffer(storedFilename) : localReadBuffer(storedFilename);
}
